# adapted from https://github.com/skjb/pysc2-tutorial
# changes: removed invalid actions lists as the NN wont be using it. or it could be another feature for NN
...
class SparseAgent(base_agent.BaseAgent):
    ...
    def step(self, obs):
        ...
        if obs.last():
            ...
            self.qlearn.learn(self.previous_state, self.previous_action, reward, 'terminal', True)

            self.qlearn.save_model(DATA_FILE)
        ...
        if self.move_number == 0:
            ...
            #learn from previous_action
            if self.previous_action is not None:
                self.qlearn.learn(self.previous_state, self.previous_action, 0, current_state)
            ...
            # getting new action from RL algorithm
            #rl_action = self.qlearn.act(str(current_state), excluded_actions)  # commented out as we are not including excluded actions
            rl_action = self.qlearn.act(np.array(current_state))
